...
try:
    ...
    aiz_mail_templates = aizMailTemplate.objects.all()
...
try:
    ...
    for penalty in penalties:
        ...
        if not PenaltyAccounts.objects.filter(**filter_conditions).exists():
            ...
            if apps.is_installed("attendance"):
                aiz.late_early_id = penalty.late_early_id
            if apps.is_installed("leave"):
                aiz.leave_request_id = penalty.leave_request_id
                aiz.leave_type_id = penalty.leave_type_id
                aiz.minus_leaves = penalty.minus_leaves
                aiz.deduct_from_carry_forward = penalty.deduct_from_carry_forward
            aiz.save()
