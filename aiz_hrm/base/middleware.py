...
CACHE_KEY = "aiz_company_models_cache_key"
...
class CompanyMiddleware:
    ...
    def _add_company_filter(self, model, company_id):
        ...
        is_aiz_manager = isinstance(model.objects, aizCompanyManager)
        ...
        if is_company_model:
            ...
            elif is_aiz_manager and related_company_field:
                model.add_to_class(
            ...
            for app_label, models in app_model_mappings.items():
                if apps.is_installed(app_label):
                    company_models.extend(
                        [get_aiz_model_class(app_label, model) for model in models]
                    )
