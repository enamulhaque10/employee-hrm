...
class EmployeeSectionForm(ModelForm):
    """
    EmployeeSectionForm model's form
    """
...
class JobPositionMultiForm(ModelForm):
    ...
    department_id = aizMultiSelectField(queryset=Department.objects.all())
    ...
    def clean(self):
        """
        Validate that the Employee Section does not already exist in the selected departments.
        """
    ...
    def save(self, *args, **kwargs):
        """
        Save the Employee Sections for each selected department.
        """
...
class EmployeeSelectionMultiForm(ModelForm):
    """
    EmployeeSelection model's form
    """
    ...
    def clean(self):
        ...
        cleaned_data = super().clean()
        department_ids = self.data.getlist("department_id")
        employee_section = self.data.get("employee_section")

        existing_section = EmployeeSection.objects.filter(
            department_id__in=department_ids, employee_section=employee_section
        )

        if existing_section.exists():
            existing_deps = existing_section.values_list("department_id", flat=True)
            dep_names = Department.objects.filter(id__in=existing_deps).values_list(
                "department", flat=True
            )
            raise ValidationError(
                {
                    "department_id": _("Employee Section already exists under {}").format(
                        ", ".join(dep_names)
                    )
                }
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            ...
            for dep_id in department_ids:
                dep = Department.objects.get(id=dep_id)
                if EmployeeSection.objects.filter(
                    department_id=dep, employee_section=employee_section
                ).exists():
                    messages.error(request, f"Employee Section already exists under {dep}")
                else:
                    section = EmployeeSection(department_id=dep, employee_section=employee_section)
                    section.save()
                    sections.append(section.pk)

            return EmployeeSection.objects.filter(id__in=sections)
        return super().save(*args, **kwargs)
...
class EmployeeUnitForm(ModelForm):
    """
    EmployeeUnit model's form
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if not self.instance.pk:
            self.fields["employee_section_id"] = forms.ModelMultipleChoiceField(
                queryset=self.fields["employee_section_id"].queryset
            )
            attrs = self.fields["employee_section_id"].widget.attrs
            attrs["class"] = "oh-select oh-select2 w-100"
            attrs["style"] = "height:45px;"
    ...
    def save(self, commit, *args, **kwargs) -> Any:
        if not self.instance.pk:
            request = getattr(_thread_locals, "request")
            employee_sections = EmployeeSection.objects.filter(
                id__in=self.data.getlist("employee_section_id")
            )
            units = []
            for section in employee_sections:
                unit = EmployeeUnit()
                unit.employee_section_id = section
                unit.employee_unit = self.data["employee_unit"]
                try:
                    unit.save()
                except:
                    messages.info(request, f"Role already exists under {section}")
                units.append(unit.pk)
            return EmployeeUnit.objects.filter(id__in=units)
        super().save(commit, *args, **kwargs)
...
class RotatingWorkTypeAssignForm(ModelForm):
    ...
    employee_id = aizMultiSelectField(
...
class EmployeeShiftScheduleUpdateForm(ModelForm):
    ...
    def as_p(self):
        ...
        context = {"form": self}
        table_html = render_to_string("aiz_form.html", context)
        return table_html
...
class MailTemplateForm(ModelForm):
    ...
    def get_template_language(self):
        mail_data = {
            ...
            "Receiver|Employee Category": "instance.get_employee_type",
            "Sender|Employee Category": "self.get_employee_type",
...
class CompanyLeaveForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["based_on_week"].widget.option_template_name = (
            "aiz_widgets/select_option.html"
        )
...
class PenaltyAccountForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if apps.is_installed("leave") and employee:
            LeaveType = get_aiz_model_class(app_label="leave", model="leavetype")
