...
def update_rotating_work_type_assign(rotating_work_type, new_date):
    ...
    bot = User.objects.filter(username="aiz Bot").first()
    if bot is not None:
        employee = rotating_work_type.employee_id
        notify.send(
            bot,
...
def work_type_rotate_after(rotating_work_work_type):
    """
    This method for rotate Employee Section based on after day
    """
...
def work_type_rotate_weekend(rotating_work_type):
    """
    This method for rotate Employee Section based on weekend
    """
    date_today = datetime.now()
...
def work_type_rotate_every(rotating_work_type):
    """
    This method for rotate Employee Section based on every month
    """
    date_today = datetime.now()
    switch_date = rotating_work_type.next_change_date
...
def switch_work_type():
    """
    This method change employees Employee Section information regards to the Employee Section request
    """
    from django.contrib.auth.models import User
    ...
    for request in work_type_requests:
        work_info = request.employee_id.employee_work_info
        # updating requested Employee Section to the employee work information.
...
def undo_work_type():
    """
    This method undo previous employees Employee Section information regards to the Employee Section request
    """
    from django.contrib.auth.models import User
    ...
    today = date.today()
    # here will get all the active Employee Section requests
    work_type_requests = WorkTypeRequest.objects.filter(
        canceled=False,
        approved=True,
        requested_till__lt=today,
    ...
    for request in work_type_requests:
        work_info = request.employee_id.employee_work_info
        # updating employee work information's Employee Section to previous Employee Section
