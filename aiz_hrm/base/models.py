...
from aiz.aiz_middlewares import _thread_locals
...
class JobPosition(aizModel):
    ...
    objects = aizCompanyManager("department_id__company_id")
    ...
    class Meta:
        ...
        verbose_name_plural = _("Job Position")

    def __str__(self):
        return str(self.job_position + " - (" + self.department_id.department) + ")"
    
class EmployeeSection(aizModel):
    """
    EmployeeSection model
    """

    employee_section = models.CharField(max_length=50, blank=False, null=False)
    department_id = models.ForeignKey(
        Department,
        on_delete=models.PROTECT,
    ...
    class Meta:
        ...
        verbose_name = _("Employee Section")
        verbose_name_plural = _("Employee Section")

    def __str__(self):
        return str(self.employee_section + " - (" + self.department_id.department) + ")"


class JobRole(aizModel):
    """JobRole model"""
    ...
    objects = aizCompanyManager("job_position_id__department_id__company_id")
...
class EmployeeUnit(aizModel):
    """EmployeeUnit model"""

    employee_section_id = models.ForeignKey(
        EmployeeSection, on_delete=models.PROTECT, verbose_name=_("Employee Section")
    )
    employee_unit = models.CharField(max_length=50, blank=False, null=True)
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = aizCompanyManager("employee_section_id__department_id__company_id")
    ...
    class Meta:
        ...
        verbose_name = _("Employee Unit")
        verbose_name_plural = _("Employee Units")
        unique_together = ("employee_section_id", "employee_unit")

    def __str__(self):
        return f"{self.employee_unit} - {self.employee_section_id.employee_section}"


class WorkType(aizModel):
    """
    WorkType model
    """
...
class RotatingWorkType(aizModel):
    """
    ...
    objects = aizCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(aizModel):
    """
...
class EmployeeType(aizModel):
    ...
    class Meta:
        ...
        verbose_name_plural = _("Employee Categorys")
        ...
        if request and request.POST:
            ...
            if (
                ...
                raise ValidationError(
                    "This Employee Category already exists in this company"
                )
        return
...
class EmployeeShift(aizModel):
    ...
    objects = aizCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(aizModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the aiz will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = aizCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShiftAssign(aizModel):
    """
...
class WorkTypeRequest(aizModel):
    """
...
class WorkTypeRequestComment(aizModel):
    """
...
class ShiftRequestComment(aizModel):
    """
...
class Tags(aizModel):
    ...
    objects = aizCompanyManager(related_company_field="company_id")

    def __str__(self):
        return self.title


class aizMailTemplate(aizModel):
...
class DynamicEmailConfiguration(aizModel):
    """
...
class MultipleApprovalCondition(aizModel):
...
class AnnouncementComment(aizModel):
    """
...
class DashboardEmployeeCharts(aizModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(aizModel):
...
class PenaltyAccounts(aizModel):
    """
...
def create_deduction_cutleave_from_penalty(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if apps.is_installed("payroll") and penalty_amount:
            Deduction = get_aiz_model_class(app_label="payroll", model="deduction")
            penalty = Deduction()
