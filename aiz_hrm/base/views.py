...
from aiz.methods import get_aiz_model_class, remove_dynamic_url
from aiz_audit.forms import HistoryTrackingFieldsForm
from aiz_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/aiz_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/aiz_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/aiz_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/aiz_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/aiz_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/aiz_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/aiz_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/aiz_job_position.html",
...
class aizPasswordResetView(PasswordResetView):
    """
    aiz View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    aiz View for Employee Reset Password
    """
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_aiz_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(aiz_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/aiz-logo.png")
    company_name = "aiz"
...
@login_required
@hx_request_required
@permission_required("base.change_aizmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = aizMailTemplate.objects.get(id=obj_id)
...
@login_required
@require_http_methods(["POST"])
@permission_required("base.add_aizmailtemplate")
...
@login_required
@permission_required("base.delete_aizmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = aizMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_section(request):
    """
    This method is used to view Employee Section
    """

    departments = Department.objects.all()
    sections = False
    if EmployeeSection.objects.exists():
        sections = True
    form = EmployeeSectionForm()
    if request.method == "POST":
        form = EmployeeSectionForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, _("Employee Section has been created successfully!"))
    return render(
        request,
        "base/employee_section/employee_section.html",
        {"form": form, "departments": departments, "sections": sections},
    )
...
# @hx_request_required
# @permission_required("base.add_jobposition")
def employee_section_creation(request):
    """
    This method is used to create Employee Section
    """
    dynamic = request.GET.get("dynamic") if request.GET.get("dynamic") else ""
    form = EmployeeSelectionMultiForm()
    if request.method == "POST":
        form = EmployeeSelectionMultiForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, _("Employee Section has been created successfully!"))
            return HttpResponse("<script>window.location.reload()</script>")
    return render(
        request,
        "base/employee_section/employee_section_form.html",
        {
            "form": form,
            "dynamic": dynamic,
        },
...
@login_required
def employee_section_update(request, id, **kwargs):
    """
    This method is used to update Employee Section
    args:
        id : Employee Section instance id

    """
    employee_section = EmployeeSection.find(id)
    form = EmployeeSectionForm(instance=employee_section)
    if request.method == "POST":
        form = EmployeeSectionForm(request.POST, instance=employee_section)
        if form.is_valid():
            form.save(commit=True)
            messages.success(request, _("Employee Section updated."))
            return HttpResponse("<script>window.location.reload()</script>")
    return render(
        request,
        "base/employee_section/employee_section_form.html",
        {"form": form, "employee_section": employee_section},
    )
...
@login_required
def employee_unit_create(request):
    """
    This method is used to create employee unit.
    """
    dynamic = request.GET.get("dynamic")
    form = EmployeeUnitForm()
    if request.method == "POST":
        form = EmployeeUnitForm(request.POST)
        if form.instance.pk and form.is_valid():
            form.save(commit=True)
            messages.success(request, _("Employee Unit has been created successfully!"))
        elif (
            not form.instance.pk
            and form.data.getlist("employee_section_id")
            and form.data.get("employee_unit")
        ):
            form.save(commit=True)
            messages.success(request, _("Employee Unit has been created successfully!"))
            return HttpResponse("<script>window.location.reload()</script>")
    ...
    return render(
        request,
        "base/employee_unit/employee_unit_form.html",
        {
            "form": form,
            "dynamic": dynamic,
        },
...
@login_required
def employee_unit_view(request):
    """
    This method is used to view employee unit.
    """

    sections = EmployeeSection.objects.all()
    employee_unit = False
    if EmployeeUnit.objects.exists():
        employee_unit = True

    return render(
        request,
        "base/employee_unit/employee_unit.html",
        {"employee_sections": sections, "employee_unit": employee_unit},
    )
...
@login_required
def employee_unit_update(request, id, **kwargs):
    employee_unit = EmployeeUnit.find(id)
    form = EmployeeUnitForm(instance=employee_unit)
    if request.method == "POST":
        form = EmployeeUnitForm(request.POST, instance=employee_unit)
        if form.is_valid():
            form.save(commit=True)
            messages.success(request, _("Employee Unit updated."))
            return HttpResponse("<script>window.location.reload()</script>")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            notify.send(
                ...
                recipient=users,
                verb="You are added to rotating Job Position",
            ...
            messages.success(request, _("Rotating Job Position assigned."))
            response = render(
                request,
...
def employee_type_view(request):
    """
    This method is used to view Employee Category
    """
...
def employee_type_create(request):
    """
    This method is used to create Employee Category
    """
    dynamic = request.GET.get("dynamic")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            form = EmployeeTypeForm()
            messages.success(request, _("Employee Category created."))
            return HttpResponse("<script>window.location.reload();</script>")
    return render(
        request,
...
def employee_type_update(request, id, **kwargs):
    """
    This method is used to update Employee Category instance
    args:
        id : Employee Category instance id
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Employee Category updated."))
            return HttpResponse("<script>window.location.reload();</script>")
    return render(
        request,
        "base/employee_type/employee_type_form.html",
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_aiz_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_aiz_model_class(
...
@login_required
@permission_required("aiz_audit.change_accountblockunblock")
...
@login_required
@permission_required("aiz_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("aiz_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("aiz_audit.change_audittag")
...
@login_required
def get_aiz_installed_apps(request):
