...
from aiz_views.cbv_methods import login_required
...
class TimeSheetNavView(aizNavView):
    """
    Nav bar
    """
...
class TimeSheetList(aizListView):
    """
...
class TaskTimeSheet(TimeSheetList):
    ...
    def get_queryset(self):
        queryset = aizListView.get_queryset(self)
        task_id = self.kwargs.get("task_id")
...
class TimeSheetFormView(aizFormView):
    """
...
class TimeSheetCardView(aizCardView):
    """
    For card view
    """
...
class TimeSheetDetailView(aizDetailedView):
    """
