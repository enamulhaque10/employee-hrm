...
class Stage(aizModel):
    ...
    objects = aizCompanyManager(related_company_field="recruitment_id__company_id")
...
from aiz.signals import pre_bulk_update


class RejectReason(aizModel):
    """
    RejectReason
    """
...
class RejectedCandidate(aizModel):
    """
    RejectedCandidate
    """
...
class RecruitmentSurvey(aizModel):
    """
    ...
    job_position_ids = models.ManyToManyField(
        JobPosition, verbose_name=_("Job Position"), editable=False
    )
    sequence = models.IntegerField(null=True, default=0)
    ...
    objects = aizCompanyManager(related_company_field="recruitment_ids__company_id")
...
class QuestionOrdering(aizModel):
    """
...
class RecruitmentSurveyAnswer(aizModel):
    """
...
class RecruitmentMailTemplate(aizModel):
...
class SkillZoneCandidate(aizModel):
    """
...
class CandidateRating(aizModel):
...
class RecruitmentGeneralSetting(aizModel):
    """
...
class InterviewSchedule(aizModel):
    """
...
class CandidateDocumentRequest(aizModel):
    title = models.CharField(max_length=100)
