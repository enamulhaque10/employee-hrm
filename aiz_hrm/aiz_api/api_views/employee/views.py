...
from aiz.decorators import owner_can_enter
from aiz_api.api_decorators.base.decorators import permission_required
from aiz_api.api_methods.employee.methods import get_next_badge_id
...
class EmployeeTypeAPIView(APIView):
    """
    Retrieves Employee Categorys.

    Methods:
        get(request, pk=None): Returns a single Employee Category if pk is provided, otherwise returns all Employee Categorys.
    """
...
class DocumentRequestAPIView(APIView):
    ...
    @manager_permission_required("aiz_documents.add_documentrequests")
    def post(self, request):
    ...
    @manager_permission_required("aiz_documents.change_documentrequests")
    def put(self, request, pk):
        document_request = self.get_object(pk)
...
class DocumentAPIView(APIView):
    ...
    @manager_or_owner_permission_required(
        DocumentRequest, "aiz_documents.add_document"
    )
    def post(self, request):
    ...
    @method_decorator(owner_can_enter("aiz_documents.change_document", Employee))
    def put(self, request, pk):
        document = self.get_object(pk)
    ...
    @method_decorator(owner_can_enter("aiz_documents.delete_document", Employee))
    def delete(self, request, pk):
        document = self.get_object(pk)
        document.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class DocumentRequestApproveRejectView(APIView):
    ...
    @manager_permission_required("aiz_documents.add_document")
