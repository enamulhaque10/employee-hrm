...
ACCRUAL_PLAN = [("job_position", _("Job Positions")), ("job_role", _("Job Role"))]
...
class LeaverequestComment(aizModel):
    """
...
class LeaveAllocationRequest(aizModel):
    leave_type_id = models.ForeignKey(
...
class LeaveallocationrequestComment(aizModel):
    """
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaveRequest(aizModel):
        leave_type_id = models.ForeignKey(
            LeaveType, on_delete=models.PROTECT, verbose_name="Leave type"
        )
        employee_id = models.ForeignKey(
...
class LeaveGeneralSetting(aizModel):
    """
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaverequestComment(aizModel):
        """
...
class EmployeePastLeaveRestrict(aizModel):
...
if apps.is_installed("attendance"):
    ...
    class OverrideLeaveRequests(LeaveRequest):
        ...
        def leaverequest_pre_save(sender, instance, **_kwargs):
            ...
            """
            WorkRecords = get_aiz_model_class(
