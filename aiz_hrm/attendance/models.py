...
class AttendanceActivity(aizModel):
    """
...
class BatchAttendance(aizModel):
    """
...
class AttendanceRequestFile(aizModel):
    file = models.FileField(upload_to="attendance/request_files")


class AttendanceRequestComment(aizModel):
    """
...
class AttendanceOverTime(aizModel):
    """
...
class AttendanceLateComeEarlyOut(aizModel):
    """
...
class AttendanceValidationCondition(aizModel):
    """
...
class AttendanceGeneralSetting(aizModel):
    """
...
class WorkRecords(models.Model):
    ...
    def title_message(self):
        ...
        if title_message == "Absent":
            if apps.is_installed("leave"):
                LeaveRequest = get_aiz_model_class(
