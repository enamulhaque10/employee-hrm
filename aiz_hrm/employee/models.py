...
class Employee(models.Model):
    ...
    employee_permanent_address = models.TextField(max_length=200, blank=True, null=True)
    employee_permanent_address_country = models.CharField(max_length=100, blank=True, null=True)
    employee_permanent_address_state = models.CharField(max_length=100, null=True, blank=True)
    employee_permanent_address_city = models.CharField(max_length=30, null=True, blank=True)
    employee_permanent_address_zip = models.CharField(max_length=20, null=True, blank=True)
    ...
    employee_blood_group = models.CharField(
        max_length=50, blank=True, null=True, choices=blood_group, default="A+"
    )
    ...
    employee_number_of_son = models.IntegerField(blank=True, null=True)
    employee_number_of_daughter = models.IntegerField(blank=True, null=True)
    ...
    def get_employee_type(self):
        """
        This method is used to return the Employee Category of the employee
        """
        return getattr(
    ...
    def get_archive_condition(self):
        ...
        if apps.is_installed("onboarding"):
            OnboardingStage = get_aiz_model_class("onboarding", "onboardingstage")
            OnboardingTask = get_aiz_model_class("onboarding", "onboardingtask")
        ...
        if apps.is_installed("recruitment"):
            Recruitment = get_aiz_model_class("recruitment", "recruitment")
            Stage = get_aiz_model_class("recruitment", "stage")
    ...
    def check_online(self):
        ...
        """
        if apps.is_installed("attendance"):
            Attendance = get_aiz_model_class("attendance", "attendance")
...
class EmployeeWorkInformation(models.Model):
    ...
    employee_unit_id = models.ForeignKey(
        EmployeeUnit,
        on_delete=models.PROTECT,
        null=True,
        blank=True,
    ...
    last_promotion_date = models.DateField(
        null=True, blank=True, verbose_name=_("Last Promotion Date")
    )
    ...
    employee_grade = models.CharField(
        max_length=50, null=True, blank=True, verbose_name=_("Employee Grade")
    )
    service_length_in_incepta = models.CharField(
        max_length=50, null=True, blank=True, verbose_name=_("Service Length in Incepta")
    )
...
class EmployeeBankDetails(aizModel):
    """
...
class PolicyMultipleFile(aizModel):
    """
...
class DisciplinaryAction(aizModel):
    """
    Disciplinary model
    """
...
class EmployeeGeneralSetting(aizModel):
    """
...
class EventCalender(aizModel):
    ...
    event_date = models.DateTimeField(null=True, blank=True, verbose_name=_("Event Date"))
