...
from aiz_automations.methods.methods import get_model_class
from aiz_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(aizModel):
...
class KeyResult(aizModel):
    ...
    history = aizAuditLog(bases=[aizAuditInfo])
...
class EmployeeObjective(aizModel):
    ...
    history = aizAuditLog(bases=[aizAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
    objects = aizCompanyManager("employee_id__employee_work_info__company_id")
...
class Comment(models.Model):
    ...
    history = aizAuditLog(excluded_fields=["comment"], bases=[aizAuditInfo])
    objects = aizCompanyManager(
...
class QuestionTemplate(aizModel):
...
class Question(aizModel):
    ...
    objects = aizCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(aizModel):
    ...
    objects = aizCompanyManager("question_id__template_id__company_id")


class Feedback(aizModel):
...
class EmployeeBonusPoint(aizModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
